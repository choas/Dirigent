    ...
    fn handle_playbook_selection(&mut self, selected_play: Option<crate::settings::Play>) {
        ...
        if vars.is_empty() {
            match self.db.insert_global_cue(&prompt) {
                ...
                Err(e) => self.set_status_message(format!("Failed to create cue: {}", e)),
            }
            return;
        }
    ...
    fn render_lava_lamp(&mut self, ui: &mut egui::Ui, panel_rect: egui::Rect) {
        ...
        match self.settings.running_animation {
            ...
            RunningAnimation::Jujutsu => {
                let (w, h) = super::jujutsu::size(scale);
                let origin = egui::pos2(
                    panel_rect.right() - w - margin,
                    panel_rect.bottom() - h - margin,
                );
                ...
                super::jujutsu::paint_at(ui.painter(), ui.ctx(), origin, accent, is_dark, scale);
            }
            RunningAnimation::Thunderdome => {
                let (w, h) = super::thunderdome::size(scale);
                let origin = egui::pos2(
                    panel_rect.right() - w - margin,
                    panel_rect.bottom() - h - margin,
                );
