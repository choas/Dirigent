...
impl DirigentApp {
    ...
    fn render_git_log_entries(
        ...
        let lineage = self
            .git
            .hovered_graph_row
            .and_then(|row| graph_helpers::compute_branch_lineage(&self.git.graph_rows, row));
...
fn render_commit_row(ui: &mut egui::Ui, params: &CommitRowParams<'_>) -> (bool, bool) {
    ...
    if let Some(graph) = params.graph_row {
        let gctx = GraphPaintCtx {
            ...
            base_line_width: 1.5,
            dot_radius: 3.0,
            ellipsis_font_size: row_height * 0.7,
            max_visible_lanes: 6,
    ...
    let msg_budget = params.max_msg_chars.max(10);
    ...
    let label_font = egui::FontId::monospace(ui.text_style_height(&egui::TextStyle::Small) * 0.85);
    ui.painter().text(
    ...
    graph_helpers::paint_commit_badges(
        ...
        text_color,
        &lane_colors,
        params.semantic.warning,
    );

    // Hover tooltip.
    let hover_text = graph_helpers::format_commit_hover(params.commit, params.is_unpushed, false);
